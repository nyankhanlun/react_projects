    ...
    onClose={userProgressCtx.progress === 'cart' ? handleCloseCart : null}>
        ...
        <ul>
            {cartCtx.items.map(item => 
                <CartItem key={item.id} onIncrease={() => cartCtx.addItem(item)} onDecrease={()=> cartCtx.removeItem(item.id)} name={item.name} quantity={item.quantity} price={item.price}/>
            )}
        </ul>
        <p className="cart-total">{currencyFormatter.format(cartTotlal)}</p>
